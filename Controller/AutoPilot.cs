...
namespace RocketPal.Controller
{
    public class AutoPilot
    {
        ...
        public RocketLeagueKeyboardController KeyboardController { get; set; }
        ...
        public void DriveBlindlyUntilBallIsFound()
        {
            if (!this.drivingBlind && engaged)
            {
               ...
               // this.KeyboardController.BoostEngaged = true;
        ...
        {
            this.KeyboardController = new RocketLeagueKeyboardController();
            KeyboardController.EnableSteeringAndThrottle();
        }
        ...
        public void Disengage()
        {
            ...
            {
                this.KeyboardController.Connected = false;
        ...
        public void AvoidIdle()
        {
            if(!this.avoidingIdle)
            {
                ...
                this.KeyboardController.Throttle = .3f;
                this.KeyboardController.SteeringPosition = -.8f;
            }
        }
        ...
        private void AiWorkerThread(object sender, DoWorkEventArgs args)
        {
            ...
            while (engaged)
            {
                if (this.GameInstance.CarFound() != true && this.avoidingIdle)
                {
                    ...
                    this.KeyboardController.SteeringPosition += (float)increment;   
                ...
                {   
                    ...
                    try
                    {
                        ...
                        KeyboardController.Throttle = (distance < 1000 && Math.Abs(error) > 45) ? (distance/1000f) : 1;

                        if (Math.Abs(error) < 15)
                        {
                            KeyboardController.Throttle = 1f;

                            if (distance > 4500 || distance < 300)
                            {
                                this.KeyboardController.FrontFlip();
                            }
                            else
                            {
                                KeyboardController.BoostEngaged = true;
                            }
                        }
                        else
                        {
                            KeyboardController.BoostEngaged = false;
                        }
                        ...
                        if (error > 0)
                        {
                            ...
                            KeyboardController.SteeringPosition = (dError*this.DerivativeConstant) +
        ...
        public void ApplyPowerSlideIfNecessary(float error)
        {
            ...
            {
                this.KeyboardController.HandBrakeEngaged(true);
            }
            else
            {
                this.KeyboardController.HandBrakeEngaged(false);
            }
        }
